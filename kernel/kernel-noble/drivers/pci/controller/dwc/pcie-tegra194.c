...
struct tegra_pcie_dw {
	...
	struct gpio_desc *pex_sub_device_rst_gpiod;
	struct gpio_desc *pex_sub_device_second_rst_gpiod;
...
static int tegra_pcie_dw_parse_dt(struct tegra_pcie_dw *pcie)
{
	...
	pcie->pex_sub_device_rst_gpiod = devm_gpiod_get_optional(pcie->dev, "nvidia,sub-rst", GPIOD_OUT_LOW);
	if (IS_ERR(pcie->pex_sub_device_rst_gpiod)) {
		int err = PTR_ERR(pcie->pex_sub_device_rst_gpiod);
		dev_info(pcie->dev, "PCIe pex_sub_device_rst_gpiod err=%d\n", err);
		if (err == -EPROBE_DEFER) {
			return err;
		}

		dev_info(pcie->dev, "Not found PCIe sub devices reset GPIO: %d, ignore\n", err);
		pcie->pex_sub_device_rst_gpiod = NULL;
	}
	pcie->pex_sub_device_second_rst_gpiod = devm_gpiod_get_optional(pcie->dev, "nvidia,sub-brst", GPIOD_OUT_LOW);
	if (IS_ERR(pcie->pex_sub_device_second_rst_gpiod)) {
		int err = PTR_ERR(pcie->pex_sub_device_second_rst_gpiod);
		dev_info(pcie->dev, "PCIe pex_sub_device_second_rst_gpiod err=%d\n", err);
		if (err == -EPROBE_DEFER) {
			return err;
		}

		dev_info(pcie->dev, "Not found PCIe sub devices second reset GPIO: %d, ignore\n", err);
		pcie->pex_sub_device_second_rst_gpiod = NULL;
	}
	if (pcie->pex_sub_device_rst_gpiod && ! IS_ERR(pcie->pex_sub_device_rst_gpiod)) {
		dev_info(pcie->dev, "pex_sub_device_rst_gpiod get ready\n");
	}
	if (pcie->pex_sub_device_second_rst_gpiod && ! IS_ERR(pcie->pex_sub_device_second_rst_gpiod)) {
		dev_info(pcie->dev, "pex_sub_device_second_rst_gpiod get ready\n");
	}
...
static int tegra_pcie_dw_probe(struct platform_device *pdev)
{
	...
	if (pcie->pex_sub_device_rst_gpiod && ! IS_ERR(pcie->pex_sub_device_rst_gpiod)) {
		gpiod_set_value_cansleep(pcie->pex_sub_device_rst_gpiod, 1);
		dev_info(dev, "pex_sub_device_rst_gpiod set enable\n");
	}
	if (pcie->pex_sub_device_second_rst_gpiod && ! IS_ERR(pcie->pex_sub_device_second_rst_gpiod)) {
		gpiod_set_value_cansleep(pcie->pex_sub_device_second_rst_gpiod, 1);
		dev_info(dev, "pex_sub_device_second_rst_gpiod set enable\n");
	}
